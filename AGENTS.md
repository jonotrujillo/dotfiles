...
- Dotfiles remain decoupled from Homebrew — they can be aware of Homebrew variables but must provide escape hatches for other environments (e.g., directory checks, `.local` files)
...
- Files ending in `.local.zsh` are machine-specific; they are gitignored and automatically picked up by the modular Zsh configuration loop if present.
